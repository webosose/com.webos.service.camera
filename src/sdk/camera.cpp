...
Camera::Camera()
    : pcam_handle_(nullptr), ncam_fd_(-1), nio_mode_(IOMODE_UNKNOWN), stcam_info_(),
      cam_state_(CAMERA_STATE_UNKNOWN), callback_map_()
{
}

int Camera::init(std::string subsystem)
{
  int retval = camera_hal_if_init(&pcam_handle_, subsystem.c_str());
...
int Camera::deinit()
{
  int retval = camera_hal_if_deinit(pcam_handle_);
  if (CAMERA_ERROR_NONE != retval)
  {
...
{
  int retval = camera_hal_if_open_device(pcam_handle_, devicenode.c_str());
  if (CAMERA_ERROR_NONE != retval)
  {
...
int Camera::close()
{
  int retval = camera_hal_if_close_device(pcam_handle_);
  if (CAMERA_ERROR_NONE != retval)
  {
...
{
  int retval = camera_hal_if_get_fd(pcam_handle_, &ncam_fd_);
  if (CAMERA_ERROR_NONE != retval)
  {
...
{
  int retval = camera_hal_if_stop_capture(pcam_handle_);
  if (CAMERA_ERROR_NONE != retval)
  {
