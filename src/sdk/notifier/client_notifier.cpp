...
int Notifier::updateDeviceList(std::string dev_type, int dev_count, camera_details_t *st_dev_list)
{
  dev_event_state_t nstatus = DEV_EVENT_STATE_NONE;

  if (dev_count_ < dev_count)
    nstatus = DEV_EVENT_STATE_PLUGGED;
  else if (dev_count_ > dev_count)
    nstatus = DEV_EVENT_STATE_UNPLUGGED;
  else
...
int Notifier::getDeviceInfo(int dev_num, camera_details_t *pst_info)
{
  int count = 0;
