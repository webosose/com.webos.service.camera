...
int UDEVClient::subscribeToClient(udevhandler_cb cb)
{
    ...
    cam_info_ = new (std::nothrow) camera_info_t[MAX_DEVICE_COUNT];
...
void UDEVClient::getDevice(struct udev_device *device,camera_info_t *caminfo)
{
    const char *str;
    dev_t devnum;
    int count;
    ...
    if (str != NULL)
    {
        ...
        if(strcmp(str,"remove") == 0)
            caminfo->cam_state = DEVICE_EVENT_STATE_UNPLUGGED;
        else if(strcmp(str,"add") == 0)
            caminfo->cam_state = DEVICE_EVENT_STATE_PLUGGED;
    }
