...
bool CameraService::startCamera(LSMessage &message)
{
    ...
    if (n_invalid_id == ndevhandle)
    {
        PMLOG_INFO(CONST_MODULE_LUNA, "DEVICE_ERROR_JSON_PARSING");
        err_id = DEVICE_ERROR_JSON_PARSING;
        obj_startcamera.setMethodReply(CONST_PARAM_VALUE_FALSE, (int)err_id, getErrorString(err_id));
    }
    else
    {
        PMLOG_INFO(CONST_MODULE_LUNA, "startCamera() ndevhandle : %d" , ndevhandle);
        // start preview here
        int key = 0;
        ...
        {
            err_id = CommandManager::getInstance().startCamera(ndevhandle, memType.str_memorytype, &key,
                                                               this->get(), CONST_EVENT_KEY_STREAMING_FAULT);

            if (DEVICE_OK != err_id)
            {
                PMLOG_DEBUG("err_id != DEVICE_OK\n");
                obj_startcamera.setMethodReply(CONST_PARAM_VALUE_FALSE, (int)err_id,
                                               getErrorString(err_id));
            }
            else
            {
                PMLOG_DEBUG("err_id == DEVICE_OK\n");
                obj_startcamera.setMethodReply(CONST_PARAM_VALUE_TRUE, (int)err_id,
                                               getErrorString(err_id));
        ...
        {
            PMLOG_INFO(CONST_MODULE_LUNA, "startCamera() memory type is not supported\n");
...
bool CameraService::stopCamera(LSMessage &message)
{
  ...
  if (n_invalid_id == ndevhandle)
  {
    PMLOG_INFO(CONST_MODULE_LUNA, "DEVICE_ERROR_JSON_PARSING");
    err_id = DEVICE_ERROR_JSON_PARSING;
    obj_stopcamera.setMethodReply(CONST_PARAM_VALUE_FALSE, (int)err_id, getErrorString(err_id));
  }
  else
  {
    PMLOG_INFO(CONST_MODULE_LUNA, "ndevhandle %d\n", ndevhandle);
    ...
    {
      PMLOG_DEBUG("err_id == DEVICE_OK\n");
      obj_stopcamera.setMethodReply(CONST_PARAM_VALUE_TRUE, (int)err_id, getErrorString(err_id));
    }
  }
    ...
    else
    {
            ...
            memType.str_memorytype == kMemtypePosixshm)
        {
            err_id = CommandManager::getInstance().startPreview(ndevhandle, memType.str_memorytype, &key,
                                                                dispType.str_window_id, &media_id,
                                                                this->get(), CONST_EVENT_KEY_STREAMING_FAULT);
