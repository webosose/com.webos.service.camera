...
LGCameraSolutionFaceDetection::FaceDetectionOpenCV(CameraSolutionManager *mgr)
    : CameraSolution(mgr), mDone(false), isThreadRunning(false), processingDone(false),
      needInputRefresh(false), internalBuffer(NULL)
...
void LGCameraSolutionFaceDetection::processForPreview(buffer_t inBuf, stream_format_t streamformat)
{
    ...
    if (needInputRefresh == true)
    {
        ...
        {
            int framesize  = streamformat.stream_width * streamformat.stream_height * 4 + 1024;
    ...
    if (needThread() && isThreadRunning == false)
    {
...
void LGCameraSolutionFaceDetection::faceDetectionProcessing()
{
    ...
    while (!mDone && isEnabled())
    {
        ...
        try
        {
            ...
            dumpFrame(internalBuffer, srcBuf.stream_width, srcBuf.stream_height, inputSize,
                      filename, filepath);
#endif
...
void LGCameraSolutionFaceDetection::print()
{
    ...
    {
        PMLOG_INFO(CONST_MODULE_NAME, "print face[%d] (%d %d) %dx%d\n", i, mFaces[i].x, mFaces[i].y,
                   mFaces[i].width, mFaces[i].height);
    }
...
int LGCameraSolutionFaceDetection::Draw(buffer_t srcframe, stream_format_t streamformat)
{
    ...
    try
    {
        ...
        {
            Point lb(faces[i].x * rx + faces[i].width * rx, faces[i].y * ry + faces[i].height * ry);
            Point tr(faces[i].x * rx, faces[i].y * ry);

            PMLOG_INFO(CONST_MODULE_NAME, "Draw face[%d] %d %d %d %d\n", i, tr.x, tr.y, lb.x - tr.x,
                       lb.y - tr.y);
...
int LGCameraSolutionFaceDetection::dumpFrame(unsigned char *inputY, int width, int height,
