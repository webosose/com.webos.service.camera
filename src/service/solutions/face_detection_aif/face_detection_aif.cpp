...
{
  return SOLUTION_FACE_DETECTION_AIF;
}
  ...
  do {
    ...
    */
    ...
    for (rj::SizeType i = 0; i < faces.Size(); i++) {
      ...
      jobject_put(jsonFaceElementObj, J_CSTR_TO_JVAL("confidence"), jnumber_create_i32(confidence));
...
{
  ai.detect( type,
             Mat(Size(oDecodedImage_.outWidth_, oDecodedImage_.outHeight_), CV_8UC3, oDecodedImage_.pImage_),
             output );
  return true;
