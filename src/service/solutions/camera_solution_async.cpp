...
struct PerformanceControl {
  ...
  void calculateFPS(void) {
    ...
    PMLOG_INFO(LOG_TAG, ">>>>>> fps : %f, target_fps : %f <<<<<<",
                          avrFPS_, targetFPS_);

    resetFPS();
  }

  void targetFPS(double targetFPS) {
...
{
  if (!isEnabled()) return;
  if (!(solutionProperty_ & LG_SOLUTION_SNAPSHOT)) return;
}

void CameraSolutionAsync::processForPreview(buffer_t inBuf)
{
  if (!isEnabled()) return;
  if (!(solutionProperty_ & LG_SOLUTION_PREVIEW)) return;
...
void CameraSolutionAsync::startThread(void)
{
  if (threadJob_ == nullptr) {
    ...
    try {
      ...
      PMLOG_INFO(LOG_TAG, "Caught a system error with code %d meaning %s",
          e.code().value(), e.what());
    }
...
void CameraSolutionAsync::notify (void)
{
  cv_.notify_all();
}
...
CameraSolutionAsync::WaitResult CameraSolutionAsync::wait (void)
{
  ...
  try {
    ...
    PMLOG_INFO (LOG_TAG, "Caught a system_error with code %d meaning %s",
        e.code().value(), e.what());
    res = ERROR;
  }
...
bool CameraSolutionAsync::checkAlive(void)
{
  return bAlive_;
}

void CameraSolutionAsync::setAlive(bool bAlive)
{
  bAlive_ = bAlive;
}
...
void CameraSolutionAsync::pushJob(buffer_t inBuf)
{
  if (queueJob_.empty()) {
    ...
    queueJob_.push(std::make_unique<Buffer>(
          (uint8_t*)inBuf.start, inBuf.length));
    notify();
  }
}
