...
const std::map<int, const char*> tblColorSpace = {
  {0, "UNKNOWN"}, {1, "GRAYSCALE"},
  {2, "RGB"},     {3, "YCbCr"},
  {4, "CMYK"},    {5, "YCCK"}
};
...
std::string FaceDetectionCNN::getSolutionStr(void)
{
    return SOLUTION_FACE_DETECTION_CNN;
}
...
void FaceDetectionCNN::processing(void)
{
    do {
      ...
      if (pEvent_) (pEvent_.load())->onDone(jsonOutObj);
...
bool FaceDetectionCNN::detectFace(void)
{
  ...
  //int* pResults =
  facedetect_cnn( pBuffer_,
                  oDecodedImage_.pImage_,
                  oDecodedImage_.outWidth_,
                  oDecodedImage_.outHeight_,
